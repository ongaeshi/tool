# module Radiko
  class Radiko
    ...
    def url
      ...
      "https://radiko.jp/share/?sid=#{@channel}&t=#{time_url}"
    end
  ...
  def radiko_url(title, channel, wday, hour, min)
    r = Radiko.new(channel, last_wday(wday + 1, hour, min))
    r.url
    # "<a href=\"#{r.url}\">#{title}</a>"
  end
# end
